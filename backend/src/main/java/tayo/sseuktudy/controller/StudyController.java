...
public class StudyController {
    ...
    public ResponseEntity<Map<String, Object>> getStudyByLeader(HttpServletRequest request){
        Map<String, Object> resultMap = new HashMap<>();

        HttpStatus status = null;
        logger.info("내가 만든 스터디 정보 API");
        ...
        if(!decodeUserId.equals(ACCESS_TOKEN_TIMEOUT)){
            ...
            try{
                ...
                List<StudyInfoDto> result = studyService.getStudyByLeader(decodeUserId);
                resultMap.put("message", "SUCCESS");
                resultMap.put("data", result);
                status = HttpStatus.ACCEPTED;
